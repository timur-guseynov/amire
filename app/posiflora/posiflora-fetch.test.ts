...
test("uses middlewares", async () => {
  const server = setupServer(
    rest.get(apiPath + endpointPath, (req, res, context) =>
      res(
        context.status(200),
        context.json({
          test1: req.headers.get("X-TEST-1"),
          test2: req.headers.get("X-TEST-2"),
          test3: req.headers.get("X-TEST-3"),
        })
      )
    )
  );
  ...
  const fetch = makePosifloraFetch().use(
    (req) => {
      req.headers = {
        ...req.headers,
        "X-TEST-1": "test1",
  ...
  const request: any = await fetch.do(endpointPath);

  expect(request.test1).toBe("test1");
  expect(request.test2).toBe("test2");
  expect(request.test3).toBeNull();
